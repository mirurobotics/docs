<ParamField path="config type" required>
  The config type is a required annotation that identifies the config type to which a schema belongs. Below is the syntax for annotating a schema with a [config type slug](/docs/learn/config-types#param-slug).

  <CodeGroup>

  ```yaml JSON Schema
  x-miru-config-type: "{config-type-slug}"
  ```

  
  ```cue CUE
  @miru(config_type="{config-type-slug}")
  ```

  </CodeGroup>

  Examples: `mobility`, `safety-features`, `perception`
</ParamField>

<ParamField path="instance file path">
  The instance file path is the absolute filesystem path where config instances for this schema are written on the device.

  Instance file paths control the type of file that config instances are deployed as. Currently, JSON (`.json`) and YAML (`.yaml`, `.yml`) are supported.

  This annotation is optional and defaults to `/srv/miru/configs/{config-type-slug}.json`, which writes config instances to `/srv/miru/configs/{config-type-slug}.json` on a given device.

  <CodeGroup>

  ```yaml JSON Schema
  x-miru-instance-filepath: "{instance-file-path}"
  ```

  
  ```cue CUE
  @miru(instance_filepath="{instance-file-path}")
  ```

  </CodeGroup>

  Examples: `/srv/miru/configs/v1/mobility.json`, `/srv/miru/configs/safety.yaml`
</ParamField>