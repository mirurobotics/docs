...
- [ ] Milestone 6 — Validation, render check, push, PR, CI green, leave draft. **Not started — deliberately out of scope for this execution session; the caller handles publishing and CI.** Carried forward for whoever runs it: (a) correct the `v0.10.4` heading and date in `docs/changelog/cli.mdx` against the real `cli-private` tag; (b) the draft gate has **three** parts, not one — (i) `core` annotation support, already merged (#150, #151, #152); (ii) the CLI wiring merged and tagged in `cli-private`, still unmerged on `origin/feat/instance-slots-annotation`; and (iii) a backend CLI API surface that accepts `instance_slots`, which does not yet exist, so a new CLI API version is required (see Surprises & Discoveries); (c) the PR body must still state the CLI release dependency and note the pre-existing red `audit` job with `GHSA-5p4m-2wfm-xmqj`; (d) the `pnpm dev` render check has not been performed, so the new `<CodeGroup>` tabs are verified only by the `grep -c '^  \$'` count of `8`.
...
- **2026-08-10 — CUE slots (#152) merged since authoring.** `core` `origin/main` head is now `324cd28` ("feat(schemas/cue): parse instance_slots on the CUE surface (#152)"), above `f1be215` (#151) and `9d9845d` (#150). All three language surfaces are therefore on `core` `main`. The CLI wiring remains **unmerged** on `cli-private` `origin/feat/instance-slots-annotation` (head `ae7c635`); `cli-private` `go.mod` still pins `github.com/mirurobotics/core v0.9.1` and its newest tag is still `v0.10.4-beta.2`. The draft gate still applies to the CLI wiring — and, per the entry below, to the backend CLI API surface as well.
- **2026-08-10 — The draft gate has a third part: no backend CLI API surface accepts `instance_slots`.** The Milestone 6 note originally said the gate rested on the CLI wiring alone once `core` #152 merged. That is incomplete. `backend` `origin/main` ships exactly two CLI specs, `api/specs/cli/v04.yaml` and `api/specs/cli/v05.yaml`, and neither mentions `instance_slots` — the only spec occurrences are in `api/specs/frontend/v08.yaml`. On the CLI request path, `internal/servers/cli/v05/request/config_schema.go:43` reads `return option.Map(r.InstanceFilepath.AsOpt(), cfgschdmn.NewDefaultSlots)`, deriving the slots solely from `instance_filepath`. Releasing the CLI wiring alone would therefore **not** make this documentation true: an author who writes `x-miru-instance-slots`, `instance_slots`, or `@miru_instance_slot` would silently get a single default slot at the default path. A new CLI API version that accepts `instance_slots` is required before this PR leaves draft. The gate is thus (a) `core` annotation support — merged; (b) CLI wiring merged and tagged in `cli-private` — not yet; (c) a backend CLI API surface accepting `instance_slots` — not yet, and not yet specified.
...
- **2026-08-10 (execution) — The draft gate is three-part, not two-part, and no later note may shorten it.** Wherever this plan implies the gate is satisfied once `core` #152 and the CLI wiring land — including the Milestone 0 concrete step's "becomes trivially satisfiable" — read the gate as: (a) `core` annotation support, **merged**; (b) the CLI wiring merged and tagged in `cli-private`, **not yet**; (c) a backend CLI API surface that accepts `instance_slots`, **not yet, and not yet specified**. As of 2026-08-10 neither `backend` `api/specs/cli/v04.yaml` nor `v05.yaml` mentions `instance_slots`, and `internal/servers/cli/v05/request/config_schema.go` derives slots from `instance_filepath` alone, so a new CLI API version is required. See Surprises & Discoveries for the evidence.
...
- `http://localhost:3000/cfg-mgmt/primitives/schemas/languages/cue` — the `@miru_instance_slot` section.
...
19. The CUE section says `@miru_instance_slot` sits alongside `@miru` and is not one of its arguments, and states that `description=""` is rejected in CUE but accepted in JSON Schema and Opaque.
