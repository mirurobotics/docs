...
Archiving a release retires it. It's hidden from the releases list, and no new devices can adopt it. Devices already running the release keep it and can still receive deployments from it, so you can change their configs without upgrading them.

Deploying the release to any other device is blocked, including rolling a device back to it. To do that, unarchive the release first.

A release can't be archived while it has [staged or drifted deployments](/cfg-mgmt/deploy/staging-area#archive-a-deployment). Archive those first.
