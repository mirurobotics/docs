...
Configurations are deployed to devices via the Miru Agent, a lightweight background service (a `systemd` service on Linux, a Windows service on Windows) that runs on your devices, handling the deployment lifecycle of configurations. Visit the [Miru Agent](/developers/agent/overview) documentation for more information.
...
<ParamField path="os" type="enum">
  <MutableBadge /> {' '} <NullableBadge />

  The operating system family of the device. Reported by the agent; empty until the agent reports it.

  Allowed values: `linux`, `windows`
</ParamField>

<ParamField path="arch" type="enum">
  <MutableBadge /> {' '} <NullableBadge />

  The CPU architecture of the device. Reported by the agent; empty until the agent reports it.
...
<ParamField path="hostname" type="string">
  ...
  The hostname of the device. Reported by the agent; empty until the agent reports it.

  Examples: `robot-11`
</ParamField>

<ParamField path="os_version" type="string">
  <MutableBadge /> {' '} <NullableBadge />

  The human-readable operating system version of the device. Reported by the agent; empty until the agent reports it.

  Examples: `Ubuntu 22.04`, `Windows 11 Pro`
</ParamField>

<ParamField path="kernel_version" type="string">
  <MutableBadge /> {' '} <NullableBadge />

  The operating system kernel version of the device. Reported by the agent; empty until the agent reports it.

  Examples: `5.15.0-91-generic`
</ParamField>
