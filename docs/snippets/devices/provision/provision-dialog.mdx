...
<Note>
  The dashboard generates a Linux command. On Windows, copy the token (the value after `MIRU_PROVISIONING_TOKEN=`) and within 5 minutes run the Windows command from [provisioning tokens](/provision-devices/provisioning-tokens#provision-the-device) in PowerShell opened as administrator.
</Note>
