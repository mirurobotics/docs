A rule's optional `upload` block declares which registered [bucket](/data-uploads/primitives/buckets) matching files are written to, and the object path within it. Omit it for a rule that only manages local retention.
