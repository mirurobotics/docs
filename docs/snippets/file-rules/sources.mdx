...
<ParamField path="stability_window_secs" type="integer" default="60">
  ...
  unchanged (quiescent) before it is considered finished. Going quiescent is what makes
  a file eligible for upload, and it is also when the rule's retention guarantee ends.
