...
`@miru_instance_slot` attributes are the exception: they are collected from every file in the package, so they may be declared on any of them. Keep them all in one file so their order stays predictable, since the instance format is inferred from the first slot's file path.
