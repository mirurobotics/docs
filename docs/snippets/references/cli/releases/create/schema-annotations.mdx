...
<ParamField path="instance file path">
  ...
  <Warning>
    On Linux, the Miru Agent ships with out-of-the-box access to `/srv/miru`. To deploy configs to a custom directory, please visit the [File system access](/developers/agent/filesys-access#configs) page.
  </Warning>

 This annotation is optional and defaults to `/srv/miru/configs/{config-type-slug}.{instance-format}`. The default is a Linux path, so schemas for Windows devices must set an explicit path such as `C:\ProgramData\Robot\configs\mobility.json`. To share the same schema amongst multiple config instances, declare [instance slots](#param-instance-slots) instead.
  ...
  Examples: 
      ...
      - `C:\ProgramData\Robot\configs\mobility.json`

  **Path rules**

  - Paths must be absolute: a Linux path starts with `/`, and a Windows path starts with a drive letter, `:`, and `\` or `/` (for example, `C:\`). UNC and device paths (`\\server\share`, `\\?\`), drive-relative paths (`C:foo`), and root-relative paths (`\foo`) are rejected with `path_not_absolute`.
  - Windows paths may not contain `<`, `>`, `"`, `|`, `?`, `*`, a `:` after the drive, or a path segment ending in `.` or a space. These are rejected with `invalid_filepath`.
  - Windows paths are compared case-insensitively, so `C:\Robot\a.json` and `c:\robot\A.json` are the same path.
  - File extensions are case-sensitive: use `.json`, `.yaml`, or `.yml`, not `.JSON`.
...
<ParamField path="instance slots">
  ...
  All slots in a schema must target the same operating system: either all Linux paths or all Windows paths. Mixing them fails with `mixed_config_schema_slot_os`.
