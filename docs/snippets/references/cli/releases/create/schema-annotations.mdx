<ParamField path="config type" required>
  The config type is a required annotation that identifies the config type to which a schema belongs. Below is the syntax for annotating a schema with a [config type slug](/cfg-mgmt/config-types#param-slug).
  ...
  If the provided config type slug does not yet exist, it is automatically created. To edit a config type after creation, visit the [config types documentation](/cfg-mgmt/config-types#edit-a-config-type).
...
<ParamField path="instance file path">
  ...
  <Warning>
    The Miru Agent ships with out-of-the-box access to `/srv/miru`. To deploy configurations to a custom directory, please visit the [File system access](/developers/agent/filesys-access#writing-config-instances) page.
  </Warning>
