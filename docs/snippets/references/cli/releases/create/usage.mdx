<Tabs>
  ...
  <Tab title="Upload Rules">
    Use the `--upload-rule` flag to include [upload rule](/data-uploads/releasing-upload-rules) files in the release, or `--upload-rules` for a directory of rule files. Upload rules ride along with the release's config schemas.
