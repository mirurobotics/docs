...
<ParamField path="type" type="enum">
  ...
  The user's [type](/admin/users/access-control#user-types), which sets their broad
  access level in the workspace.
...
<ParamField path="roles" type="[]enum">
  ...
  The user's workspace [roles](/admin/users/access-control#roles). Members have at
  least one role; admins and owners have none. Roles within a group are granted
  through [group membership](/concepts/groups/members).
