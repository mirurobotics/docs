...
The `everglades` release introduces device groups and adds group membership to devices, so you can build, read, and filter your fleet by the same hierarchy you manage in the dashboard. 
...
<Dropdown title="Devices">
  - <POST /> `/devices/move/bulk` — move devices between groups
</Dropdown>
...
<Separator />
<Dropdown title="Assigning devices to groups">

  <POST /> `/devices`

  A device can be assigned to a group when it is created.
  ...
  <POST /> `/devices/move/bulk`
  
  Existing devices are reassigned in bulk: <POST /> `/devices/move/bulk` moves multiple devices in one request and returns the moved devices. Send `"target_group_id": null` to unassign them instead.

  ```diff
  + POST /devices/move/bulk
    {
      "device_ids": ["dvc_123", "dvc_456"],
...
<Steps>
  ...
  <Step title="Optionally integrate new endpoints">
    Optionally integrate the new group and device move endpoints listed above. Creating, renaming, and moving groups requires the `groups:write` scope, and moving devices requires the `devices:write` scope. Bulk device moves are authorized against the target group, so grant `devices:write` on the group the devices are moving into. Add these scopes to your [API keys](/admin/apikeys) before calling those endpoints, and see [authorization](/developers/platform-api/authz) for the full scope reference.
  </Step>
