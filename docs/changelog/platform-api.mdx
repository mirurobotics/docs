...
The `everglades` release introduces device groups, allowing you to build your fleet hierarchy via API and access device group information. Groups themselves are fully manageable: create, rename, and move them as desired. A device's membership in a group is read-only, so assigning a device to a group remains a dashboard flow.
...
<Dropdown title="Config schema instance slots">
  The config schema `instance_filepath` field is replaced by the `instance_slots` field.  Instance slots specify an array of file system destinations, allowing multiple config instances to share the same schema validation. Visit the [instance slots](/cfg-mgmt/concepts/schemas/annotations#instance-slots) for the full field reference.
...
<Dropdown title="Device group membership">
  Devices now include the [group](/concepts/groups/overview) they belong to. `group_id` is always present and is `null` for unassigned devices.
...
<Dropdown title="Opaque schema language">
  `opaque` is added to the config schema `language` enum. An opaque schema carries metadata only and treats every config instance as valid. Visit the [opaque schema language](/cfg-mgmt/concepts/schemas/languages/opaque) documentation for details.
...
<Dropdown title="XML and plain text instance formats">
  `xml` and `text` are added to the config instance `format` enum, in both config instance responses and the <POST /> `/config_instances` request body. Visit the [config instance file formats](/cfg-mgmt/concepts/config-instances#file-formats) documentation for which formats each schema language supports.
...
<Dropdown title="Activation tokens replaced by provisioning tokens">
  ...
  Provisioning tokens are not scoped to a pre-existing device — create a token first, then use it to provision a new device or activate an existing device. Visit the [provisioning tokens](/provision-devices/provisioning-tokens) documentation for more details.
</Dropdown>
...
<Steps>
  ...
  <Step title="Migrate from activation tokens to provisioning tokens">
    Migrate install workflows to use [provisioning tokens](/provision-devices/provisioning-tokens) instead of activation tokens.
  </Step>
  <Step title="Optionally adopt new additive surface">
...
<Dropdown title="Deployment activity status">
  ...
  * Removed `removing` and `validating`—deployments now transition directly from `deployed` to `archived`.
  ...
  These changes are also reflected in the deployment's [status](/concepts/deployments/status#status) field.
</Dropdown>
