...
<Dropdown title="Activation tokens replaced by provisioning tokens">
  ...
  Provisioning tokens are not scoped to a pre-existing device — create a token first, then use it to provision a new device or activate an existing device. Visit the [provisioning tokens](/learn/devices/provision/provisioning-tokens) documentation for more details.
...
<Steps>
  ...
  <Step title="Migrate from activation tokens to provisioning tokens">
    Migrate install workflows to use [provisioning tokens](/learn/devices/provision/provisioning-tokens) instead of activation tokens.
  </Step>
