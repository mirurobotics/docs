...
<Steps>
  ...
  <Step title="Optionally integrate new endpoints">
    Optionally integrate the new group and device move endpoints listed above. Creating, renaming, and moving groups requires the `groups:write` scope, and moving devices requires the `devices:move` scope. Add these scopes to your [API keys](/admin/apikeys) before calling those endpoints, and see [authorization](/developers/platform-api/authz) for the full scope reference.
  </Step>
