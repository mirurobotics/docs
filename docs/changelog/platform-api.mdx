...
<Dropdown title="Device group membership">
  Devices now include the [group](/concepts/groups) they belong to. `group_id` is always present and is `null` for unassigned devices.
...
<Dropdown title="Deployment activity status">
  ...
  * Removed `removing` and `validating`—deployments now transition directly from `deployed` to `archived`.
  ...
  These changes are also reflected in the deployment's [status](/concepts/deployments#status) field.
</Dropdown>
