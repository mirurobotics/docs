...
<Dropdown title="Config schema instance slots">
  The config schema `instance_filepath` field is replaced by the `instance_slots` field.  Instance slots specify an array of file system destinations, allowing multiple config instances to share the same schema validation. Visit the [instance slots](/cfg-mgmt/concepts/schemas/instance-slots) for the full field reference.
