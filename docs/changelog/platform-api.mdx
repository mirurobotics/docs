...
The `everglades` release introduces device groups and adds group membership to devices, so you can build, read, and filter your fleet by the same hierarchy you manage in the dashboard. It also brings the Platform API up to date with three months of configuration changes: config schemas now declare instance slots instead of a single instance file path, config instances carry a slot key, deployments report when they were queued, deployed, and archived, and devices report an `archived` status.
...
<Dropdown title="Devices">
  - <POST /> `/devices/move/bulk` — move devices into a group
</Dropdown>
...
<Dropdown title="Config type slug is immutable">
  ...
  <PATCH /> `/config_types/{config_type_id}` no longer accepts a `slug` field — slugs are fixed when the config type is created.
...
<Separator />
<Dropdown title="Group management">

  Groups can be created, renamed, and moved through the API. All three endpoints require the `groups:write` scope.

  Create a group at the top level by omitting `parent_id`, or nest it under an existing group by sending one.
  ...
  <PATCH /> `/groups/{group_id}` accepts a `name` and nothing else. To change where a group sits in the hierarchy, use <POST /> `/groups/{group_id}/move` and send the new `parent_id`, or `null` to move the group to the top level.
  ...
  There is no endpoint for deleting a group. Delete groups from the [dashboard](/primitives/groups#delete-a-group).
...
<Dropdown title="Assigning devices to groups">
  ...
  Existing devices are reassigned in bulk. <POST /> `/devices/move/bulk` moves up to 1000 devices in one request and returns the moved devices. Send `"target_group_id": null` to unassign them instead.
  ...
  Bulk moves require the `devices:move` scope rather than `devices:write`. The permission is checked against the target group, so the API key must be granted `devices:move` on that group. Moving devices out of every group, by sending `"target_group_id": null`, requires a workspace-level grant.
...
<Separator />
<Dropdown title="Deployment parent">
  ...
  The full parent deployment can be expanded, on both the retrieve and list endpoints.
...
<Dropdown title="Deployment lifecycle timestamps">
  ...
  `created_at` and `updated_at` only record when the deployment record changed. Use these fields to tell when a deployment actually reached a device, or to measure how long a rollout took.
