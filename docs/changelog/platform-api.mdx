...
<Dropdown title="Device group membership">
  ...
  Membership is read-only over the API. `group_id` is returned, expandable, and filterable, but it cannot be set on <POST /> `/devices` or changed on <PATCH /> `/devices/{device_id}`. Assign and move devices from the dashboard.
