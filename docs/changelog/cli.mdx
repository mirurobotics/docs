...
CLI `v0.11.0` renames the upload rule to the **file rule**. A file rule still says which files on a device to match, but uploading them and reclaiming their disk space are now two independent, optional halves of one object. See [file rules](/data-uploads/primitives/file-rules).
...
CLI `v0.10.2` adds support for including [upload rules](/data-uploads/primitives/file-rules) in a release, letting you define data uploads alongside your config schemas.
