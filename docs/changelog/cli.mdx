...
The rule file YAML has been restructured. `name` is new and required, `collection_slug` moves under the new optional `upload` block, `destination` is replaced by `upload`, `delete_policy` is removed in favor of the new optional `retention` block, and `path` must now contain `{upload_id}`:
...
Both `upload` and `retention` are optional. Omit `upload` for a rule that only reclaims local disk; omit `retention` and the device keeps matching files indefinitely. `ttl_secs` is required inside `retention`, and `require_upload` must be set exactly when the rule has an `upload` block.

</Dropdown>

- The `upload_rules:*` API key scopes are now `file_rules:*`, so a CI pipeline creating a release with file rules needs `file_rules:manage`
- Rules are keyed by `name`, which must be unique across the rule files included in one release; two rules may now share an upload collection slug
