...
To learn more, visit the [provisioning tokens](/learn/devices/provision/provisioning-tokens) documentation.

## Breaking changes

- Provisioning via the provisioning script is not supported for provisioning `miru-agent` versions `v0.9.0` or later; use [provisioning tokens](/learn/devices/provision/provisioning-tokens) instead
<Warning>
    The legacy [provisioning script](https://github.com/mirurobotics/agent/blob/main/scripts/install/provision.sh) will refuse to install `v0.9.0` or later; follow the updated [install instructions](/developers/agent/install)
</Warning>
...
- Resources fetched via the [Device API](/developers/device-api) are fetched from Miru servers if not found locally
