...
To learn more, visit the [provisioning tokens](/fundamentals/devices/provision/provisioning-tokens) documentation.

## Breaking changes

- The legacy provisioning script only supports provisioning `miru-agent` versions that precede `v0.9.0`; use [provisioning tokens](/fundamentals/devices/provision/provisioning-tokens) for `miru-agent` versions `v0.9.0` or later
