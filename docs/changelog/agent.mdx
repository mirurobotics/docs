...
- Retention [file rule](/data-uploads/concepts/file-rules/overview) delete jobs now stop after 10 failed attempts instead of retrying forever, leaving the file on disk
...
- Added support for the latest [file rule](/data-uploads/concepts/file-rules/overview) capabilities, including retention-only rules and a configurable delay period before deletion
...
To learn more, visit the [provisioning tokens](/provision-devices/provisioning-tokens) documentation.

## Breaking changes

- The legacy provisioning script only supports provisioning `miru-agent` versions that precede `v0.9.0`; use [provisioning tokens](/provision-devices/provisioning-tokens) for `miru-agent` versions `v0.9.0` or later
