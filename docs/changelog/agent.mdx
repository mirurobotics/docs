...
Reading files for upload may require granting the `miru` user [read access](/developers/agent/filesys-access#reading-files-to-upload) to the source paths. To learn more, visit the [data uploads](/data-uploads/overview) documentation.

## Features

- Added data uploads: the agent scans for files matching an upload rule's `source.glob` and uploads them to your configured AWS S3 or GCS bucket
- Upload rules can delete source files after a successful upload when their `delete_policy` is set to `after_upload`
- The upload queue is persisted to disk, so pending uploads survive agent restarts
- Uploads are retried and requeued on transient failures, and dropped immediately on terminal (`4xx`) backend rejections
- Uploaded objects are stamped with backend-vended metadata
- Added scan and upload tracing logs for runtime observability

## Fixes

- Redact the MQTT password in logs to prevent credential leakage
- Fixed an errant upload rule ID being set from the upload collection ID
...
To learn more, visit the [file system access](/developers/agent/filesys-access) documentation.
