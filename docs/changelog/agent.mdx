...
- Added support for the latest [file rule](/data-uploads/primitives/file-rules) capabilities, including retention-only rules and a configurable delay period before deletion.

## Fixes

- Fixed a bug where failing to shutdown one component of the agent would block other unrelated components from gracefully shutting down
