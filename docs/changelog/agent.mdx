...
- Retention [file rule](/data-uploads/primitives/file-rules/overview) delete jobs now stop after 10 failed attempts instead of retrying forever, leaving the file on disk
...
- Added support for the latest [file rule](/data-uploads/primitives/file-rules/overview) capabilities, including retention-only rules and a configurable delay period before deletion
