...
<Update label="May 12, 2026">
    ...
    Device provisioning has been rebuilt around provisioning tokens, replacing the legacy [provisioning script](/learn/devices/provision/provisioning-script) workflow. Provisioning tokens are short-lived (5 minute), single-use credentials which can both create and activate devices for your workspace.

    To support provisioning tokens, we've introduced:

    - Platform API `2026-05-06.rainier` - programmatically mint provisioning tokens for automated workflows

    - Miru Agent v0.9.0 - bundles provisioning logic inside the `miru-agent` Debian package, removing the need to rely on the [provisioning script](/learn/devices/provision/provisioning-script) or custom provisioning logic when installing v0.9.0 or later.

    <Framed
        image="/images/changelog/2026-05-12/provision-dialog.png"
        background="/images/changelog/2026-05-12/background.jpg"
        innerRadius="8px"
        borderWidth="18px"
    />

    [Miru Agent Changelog »](/changelog/agent#v0-9-0)

    [Platform API Changelog »](/changelog/platform-api#2026-05-06-rainier)

    [Device Provisioning Documentation »](/learn/devices/provision/overview)
    ...
    <Dropdown title="Improvements">
        ...
        <DropdownItem>
            **`removing` deployment status:** Deployments now transition through an explicit `removing` status while config instances are torn down on the device, visible across the dashboard, [Platform API](/changelog/platform-api#2026-05-06-rainier), and [Device API](/changelog/device-api).
        </DropdownItem>
        <DropdownItem>
            **Device list expansions:** Platform API device endpoints now support `expand=current_release` and `expand=current_deployment`, plus a `current_release_id` filter when listing devices. See the [Platform API changelog](/changelog/platform-api#2026-05-06-rainier) for details.
        </DropdownItem>
    </Dropdown>
    ...
    <Dropdown title="Fixes">
        <DropdownItem>
            **Agent network lockdown:** All agent network traffic is now restricted to `mirurobotics.com` and its subdomains (loopback addresses are still permitted for local testing). Devices behind a corporate proxy or on-prem mirror need to allow the `mirurobotics.com` domain before upgrading to Miru Agent `v0.9.0`.
        </DropdownItem>
        <DropdownItem>
