...
<div className="changelog-page">
  {/* INTENTIONAL: two distinct releases shipped on September 23, 2026
      ("Device Views" and "CLI Commands" below). The duplicate date label is
      deliberate — do NOT merge, rename, or re-date either <Update> block. */}

  <Update label="September 23, 2026">
    ## Device Views

    Narrow down a large fleet to the devices you care about with new filters, group by,
    and display options. Save any combination as a view and share it with your workspace.

    ### Filters and search

    Filter devices by **status**, **release**, **group**, or **agent version**. Each
    active filter shows up as a chip above the list that you can click to remove. In
    addition, you can now search devices by their name and description.

    <LazyVideo alt="Screen recording of filtering and searching devices" className="aspect-video rounded-xl" src="https://assets.mirurobotics.com/docs/changelog/26-09-23/device-filtering.mp4" />

    ### Display options

    The Display menu allows you to control how the list is laid out.

    * **Group by:** Split the list into sections by status, release, group, or
      agent version
    * **Order by:** Sort by name or status, ascending or descending
    * **Multi-select:** Adds checkboxes to each row so you can perform a bulk action
    * **Compact:** Shorter rows to fit more devices on the screen
    * **Columns:** Show or hide the release, agent, and group columns

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-09-23/display-menu.png" borderWidth="0px" innerRadius="8px" />

    ### Saved views

    Save the current search, filters, grouping, ordering, and columns as a
    view. Saved views appear as tabs at the top of the list.

    * **Shared by default:** Views are shared with your workspace unless you
      disable the option.
    * **Shareable links:** Share a view with a teammate by sending its link
    * **Per group:** Each group has its own  views, separate from
      the views on the workspace level

    Right-click a view's tab to edit, duplicate, or delete it.

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-09-23/saved-views.png" borderWidth="0px" innerRadius="8px" />

    ## Archiving

    Devices, releases, and config types can now be archived. Archiving removes
    them from the active list without deleting them.

    * **Devices:** Archiving a device ends its agent session. To unarchive it,
      [reprovision](/provision-devices/reprovision) the device
    * **Releases:** A release can be archived once it has no staged or active
      deployments
    * **Config types:** An archived config type can't be added to new releases
      until it's unarchived

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-09-23/archived.png" borderWidth="0px" innerRadius="8px" />

    ## Groups Layout

    The Groups page has been rebuilt as three resizable panes.

    * **Tree:** Navigate your groups like a file directory
    * **Devices:** View, filter, and organize the selected group's devices
    * **Details:** The group's properties and members. Members are listed
      directly in the pane, so you can see at a glance who has access to the
      group.

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-09-23/groups-layout.png" borderWidth="0px" innerRadius="8px" />
    ...
    <Dropdown title="Improvements">
      <DropdownItem>
        **Releases and config types lists:** The Releases and Config Types
        lists are rebuilt on the same list system as Devices, with search,
        display options (ordering, columns, compact rows), and multi-select.
      </DropdownItem>

      <DropdownItem>
        **Faster lists:** Lists you've visited stay cached for 15 minutes, so
        returning to them is instant. Search results start loading while you
        type, and hovering a row preloads the page it opens.
      </DropdownItem>

      <DropdownItem>
        **Short and full deployment IDs:** Every place that copies a
        deployment ID now offers both the short `DPL-XXXXX` ID and the full ID.
      </DropdownItem>

      <DropdownItem>
        **Unavailable diffs are labeled:** When a diff can't be rendered, the
        editor and compare views now say why, instead of showing the file as
        if nothing changed.
      </DropdownItem>

      <DropdownItem>
        **Security hardening:** The app now enforces a Content Security
        Policy, sends additional security headers, and publishes a
        `security.txt` for reporting vulnerabilities.
      </DropdownItem>
    </Dropdown>
    ...
    <Dropdown title="Fixes">
      <DropdownItem>
        **Reordered map keys in compare:** Comparing two devices whose configs
        list the same map keys in a different order showed no highlighting,
        even though the summary counted every change. Map entries that were
        both moved and edited now keep their diff.
      </DropdownItem>

      <DropdownItem>
        **Removed array elements:** Removing an element from an array without
        keys is now reported as one deletion, instead of a change to every
        element after it.
      </DropdownItem>
    </Dropdown>
  </Update>
  ...
  <Update label="September 23, 2026">
    ...
    The Miru CLI can now clone, stage, and validate configs from your local machine, and
    logging in no longer requires copying a token from the dashboard.

    ### New commands

    * [`miru device clone`](/references/cli/device-clone): Write a device's current config
      files to your local machine
    * [`miru deployment clone`](/references/cli/deployment-clone): Write any deployment's
      config files to your local machine
    * [`miru device stage`](/references/cli/device-stage): Stage a deployment from config
      files on your local machine
    * [`miru config validate`](/references/cli/config-validate): Check a local config file
      against a config schema
    * [`miru deployment validate`](/references/cli/deployment-validate): Check local config
      files against a release's schemas
    * [`miru schema validate`](/references/cli/schema-validate): Check a schema against the
      same rules as `miru release create`, without creating a release
    * [`miru whoami`](/references/cli/whoami): Show which user or API key the CLI is signed
      in as

    ### Browser login

    `miru login` now opens Miru in your browser to authorize the device. Check the
    machine, location, IP address, and request time before clicking **Allow**.

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-09-23/miru-login.png" borderWidth="0px" innerRadius="8px" />

    [CLI changelog »](/changelog/cli)

    ## API Keys

    API keys are easier to audit and manage with these new features:

    * **Last used:** Each key shows the date it was last used
    * **Editable scopes:** Change a key's scopes after it's been created
    * **Create page:** Creating a key moved from a dialog to its own page
    * **Description:** Add a description for each key

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-09-23/edit-api-key.png" borderWidth="0px" innerRadius="8px" />

    [API keys »](/admin/apikeys)

    ## Ansible

    Provision devices with the official [`mirurobotics.agent`](https://galaxy.ansible.com/ui/repo/published/mirurobotics/agent/)
    Ansible collection. One play installs the Miru Agent on each host and registers it in Miru.

    ```bash
    ansible-galaxy collection install mirurobotics.agent
    ```

    * **API key stays on the controller:** Only a short-lived provisioning token reaches
      each device
    * **Safe to re-run:** Hosts that are already provisioned are skipped
    * **Matched by name:** Hosts are matched to devices by name. A missing device is
      created, and an inactive one is activated

    The collection requires [Miru Agent v0.10.2](/changelog/agent#v0-10-2) or later.

    [Provision with Ansible »](/provision-devices/ansible)
  </Update>

  <Update label="August 14, 2026">
    ## Multiple Config Instances per Schema

    A single config schema can now write **several files** to a device, using
    **instance slots**. Each slot is a separate file system destination validated by the same schema.

    This allows a robot with several identical motor controllers to use one
    `motor-controller` schema instead of duplicated config types per
    controller.

    Instance slots are only supported in [Miru CLI v0.11.0](/changelog/cli#v0-11-0) or later.

    [Instance slots »](/cfg-mgmt/concepts/schemas/annotations#instance-slots)

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-08-14/instance-slots.png" background="https://assets.mirurobotics.com/docs/changelog/26-08-14/background.jpg" borderWidth="24px 0px 0px 24px" innerRadius="8px 0px 0px 0px" />

    ## Upload Rules Renamed to File Rules

    Upload rules have been renamed to **file rules**. A rule still watches a glob on
    the device, but **upload** and **retention** are independent and optional —
    a rule can upload, reclaim disk, or both.

    The new `retention` block replaces `delete_policy`. Its required `ttl_secs` field
    sets how long a file lives on the device before it is scheduled for deletion.

    File rules require [Miru CLI v0.11.0](/changelog/cli#v0-11-0) and [Miru Agent
    v0.10.1](/changelog/agent#v0-10-1) or later.

    [File rules »](/data-uploads/concepts/file-rules/overview)

    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-08-14/file-rule.png" background="https://assets.mirurobotics.com/docs/changelog/26-08-14/background.jpg" />

    <Separator />

    <Dropdown title="Improvements">
      <DropdownItem>
        **Draft mode shows diffs:** The draft mode now shows diffs between old
        values and new values while you're editing a config.
      </DropdownItem>

      <DropdownItem>
        **Git commit message truncation:** Uploaded git commit messages longer
        than 2KB are now truncated instead of throwing an error.
      </DropdownItem>
    </Dropdown>
  </Update>
  ...
  <Update label="August 6, 2026">
    ...
    * **Deploy is blocked:** Deploy and Stage buttons are disabled until the draft is clean
    ...
    <LazyVideo alt="Screen recording of live schema validation in the config editor" className="rounded-xl" aspectRatio="3840 / 2260" src="https://assets.mirurobotics.com/docs/changelog/26-08-06/schema-validation.mp4" />
    ...
    * **Line diffs:** XML and text diff line by line rather than by structure, with
    ...
    <Framed image="https://assets.mirurobotics.com/docs/changelog/26-08-06/xml-support.png" borderWidth="0px" innerRadius="8px" />
    ...
    * **Deployments:** A live feed of deployments across your fleet, filterable by
    ...
    <LazyVideo alt="Screen recording of the fleet dashboard" className="aspect-video rounded-xl" src="https://assets.mirurobotics.com/docs/changelog/26-08-06/fleet-dashboard.mp4" />
  ...
  <Update label="July 20, 2026">
    ...
    **Data Uploads** allows devices to upload files directly to a cloud storage bucket you own. First, define an [upload rule](/data-uploads/concepts/file-rules/overview), which specifies what files to collect and where to store them in your bucket. Then, the Miru Agent detects matching files on your devices and streams them straight to your bucket.
    ...
    [Define upload rules »](/data-uploads/define-file-rules)
  ...
  <Update label="June 23, 2026">
    ...
    * From any group's members dialog
    ...
    [Manage group members »](/concepts/groups/members)
  </Update>
  ...
  <Update label="May 27, 2026">
    ...
    [Groups Documentation »](/concepts/groups/overview)
  ...
  <Update label="May 12, 2026">
    ...
    * **In-place upgrades:** Miru Agent `v0.9.0` preserves user-customized settings and automatically resets on-device state on package upgrade, so `apt-get install miru-agent` upgrades in place instead of requiring a reprovision
    ...
    Device provisioning has been rebuilt around provisioning tokens, replacing the legacy [provisioning script](/provision-devices/provisioning-script) workflow. Provisioning tokens are short-lived (5-minute), single-use credentials that can both create and activate devices for your workspace.
    ...
    * [Miru Agent v0.9.0](/changelog/agent#v0-9-0): bundles provisioning logic inside the `miru-agent` Debian package, removing the need to rely on the provisioning script to install `v0.9.0` or later
    ...
    [Device Provisioning Documentation »](/provision-devices/overview)
  ...
  <Update label="December 4, 2025">
    ...
    [Read the full documentation »](/cfg-mgmt/concepts/schemas/languages/overview)
  </Update>
