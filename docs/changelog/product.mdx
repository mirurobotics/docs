...
<Update label="August 14, 2026">
    ...
    **instance slots**. Each slot is a distinct file system destination for the same
    schema, and every config instance binds to exactly one slot. A robot with several
    identical motor controllers can use a single `motor-controller` schema with one slot
    per controller instead of a near-identical config type for each one.
    ...
    - **Slot keys:** Slot keys and file paths are unique within a schema, and every
      config instance records the `slot key` of the slot it fills. The slot's
      `filepath` is where that instance is written on the device
