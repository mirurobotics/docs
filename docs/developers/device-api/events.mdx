...
The Miru Agent exposes a Server-Sent Events (SSE) stream at the `/events` endpoint that pushes deployment lifecycle notifications in real time. Instead of polling the deployments endpoint, your application can open a single, long-lived connection and react to state changes as they happen.
...
<Warning>
Ensure you have proper permissions to access the Unix socket. See [Authentication](/docs/developers/device-api/authn) for more details.
</Warning>
...
## Versioning
Server-sent events are versioned alongside the Device API. The first Device API version that supports SSE events is [v0.2.1](/docs/changelogs/device-api#v0-2-1).
...
Miru's stability guarantees for event versioning adhere to the policy specified in the [Device API versioning](/docs/developers/device-api/versions#stability-guarantees) section.
...
For the full list of event types supported by a Device API version, visit the [Event Types](/docs/references/device-api/latest/events) section in the Device API reference.
