...
The Device API SDKs do NOT support Server-Sent Events natively. To consume the SSE stream use a standard SSE client library for your language, and parse the event frames using the `event` and `data` fields (see [Event Data](#event-data)).
...
The agent sends SSE comment lines (`:heartbeat`) approximately every 30 seconds to keep the connection alive and prevent proxies or firewalls from closing idle connections. 
...
The stream begins by replaying all retained events after the given cursor, then continues to deliver live events as they occur.
