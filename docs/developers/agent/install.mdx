...
It does not cover provisioning devices for your workspace. Visit the [device provisioning](/learn/devices/provision/overview) documentation to learn how to create and register devices with your workspace.
...
The `miru-agent` package can be upgraded via `apt` or manually by downloading and installing the new package. Since both methods use Debian packages, manual installations can be upgraded via `apt` and vice versa.
...
If the `miru-agent` package you're upgrading to is `v0.9.0` or later, upgrading will automatically reset your machine's on-device state to be compatible with the new version. However, if you're upgrading to a `miru-agent` version that precedes `v0.9.0`, you MUST [reprovision](/learn/devices/provision/reprovision) the device after upgrading the `miru-agent` package.

For example, going from `v0.6.0` to `v0.8.0` requires [reprovisioning](/learn/devices/provision/reprovision) the device but going from `v0.6.0` to `v0.9.0` does not.
...
While this method circumvents the need to _download_ the Miru Agent over a network, _provisioning_ the agent still requires some level of connectivity to register the device with the Miru servers.
