...
The agent's systemd unit restricts filesystem access using `ProtectSystem=strict`, which makes the entire filesystem read-only except for explicitly allowed paths. The agent writes config instance files to absolute paths specified by the platform, so it needs write access to those target directories.

The agent's systemd unit enables `ProtectSystem=strict`, making the entire filesystem read-only except for paths listed in `ReadWritePaths`. If your config instances target paths outside the defaults, see [File System Permissions](/docs/developers/agent/file-system-permissions) for instructions on granting access.
...
The only local interface is a Unix domain socket at `/run/miru/miru.sock`, used by applications on the same device to query the agent's [REST API](/docs/developers/device-api/overview). This socket is not accessible over the network.
