...
1. The agent receives a short-lived activation token as part of the [provisioning](/docs/learn/devices/provision/dashboard) process (either from the dashboard or an API key)
...
The agent's identity is bound to the RSA key pair generated on-device during [provisioning](/docs/learn/devices/provision/dashboard). Currently, these credentials are protected at rest through filesystem permissions:
