...
1. The agent receives a short-lived activation token as part of the [provisioning](/learn/devices/provision) process (either from the dashboard or an API key)
...
The agent's identity is bound to the RSA key pair generated on-device during [provisioning](/learn/devices/provision). Currently, these credentials are protected at rest through file system permissions:
