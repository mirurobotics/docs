...
<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/comms.light.svg" alt="Agent communication protocols overview" className="block dark:hidden" />
<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/comms.dark.svg" alt="Agent communication protocols overview" className="hidden dark:block" />
...
<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/sync-cycle.light.svg" alt="Agent sync cycle sequence" className="block dark:hidden" />
<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/sync-cycle.dark.svg" alt="Agent sync cycle sequence" className="hidden dark:block" />
...
<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/mqtt-trigger.light.svg" alt="MQTT trigger sequence" className="block dark:hidden" />
<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/mqtt-trigger.dark.svg" alt="MQTT trigger sequence" className="hidden dark:block" />

**2. Manual request (REST API)**

Oftentimes it's beneficial to manually trigger a sync cycle from an on-device application to ensure that the local state is in sync with the control plane's state. A local application can request the agent perform a sync cycle via the [Device API](/developers/device-api/overview).

<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/rest-trigger.light.svg" alt="REST trigger sequence" className="block dark:hidden" />
<img src="https://assets.mirurobotics.com/docs/v04/images/agent/architecture/rest-trigger.dark.svg" alt="REST trigger sequence" className="hidden dark:block" />
...
As a final fallback, the agent performs an initial sync cycle on startup. If encountering real-time sync issues with the agent, we recommend [restarting](/developers/agent/commands#restart) the agent to manually trigger a sync cycle.

## Deployments

Miru takes a declarative approach to deployments. Each deployment has a [target state](/concepts/deployments/status#target-status) set by the control plane (what should happen) and an [activity state](/concepts/deployments/status#activity-status) tracked by the agent (what has happened).
...
- Configurations that are already deployed remain on the file system and continue to be served by the local [REST API](/developers/device-api/overview)
