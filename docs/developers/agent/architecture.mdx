...
Miru takes a declarative approach to deployments. Each deployment has a [target state](/concepts/deployments#target-status) set by the control plane (what should happen) and an [activity state](/concepts/deployments#activity-status) tracked by the agent (what has happened).
