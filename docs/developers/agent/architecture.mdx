...
5. Emits events for successful deployment transitions
...
When a deployment transitions successfully during the sync cycle, the agent emits an event. Two event types are supported:
...
Events are persisted locally in JSONL format and broadcast to any connected Server-Sent Events (SSE) clients via the `GET /v0.2/events` endpoint. This allows on-device applications to react to deployment changes in real time without polling.

For details on connecting to the SSE stream, replaying missed events, and filtering by type, see the [Events](/docs/developers/device-api/events) guide.
