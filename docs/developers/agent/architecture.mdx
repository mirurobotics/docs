...
Miru takes a declarative approach to deployments. Each deployment has a [target state](/essentials/deployments#target-status) set by the control plane (what should happen) and an [activity state](/essentials/deployments#activity-status) tracked by the agent (what has happened).
