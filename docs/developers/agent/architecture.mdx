...
Miru takes a declarative approach to deployments. Each deployment has a [target state](/primitives/deployments#target-status) set by the control plane (what should happen) and an [activity state](/primitives/deployments#activity-status) tracked by the agent (what has happened).
