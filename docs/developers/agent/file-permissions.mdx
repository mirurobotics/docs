...
<Tabs>
  ...
  <Tab title="File doesn't exist">
    If the input file does not exist yet, the script will skip the file permission checks and check the parent/ancestor directory permissions required to create it. 

    As long as the directory has the required permissions, the Miru Agent will be able to create the file inside the directory with the proper permissions.
  ...
  <Tab title="Directory">
    Testing a directory checks that the `miru` user has the required permissions to create files inside the directory.
