...
On Linux the Miru Agent runs as the `miru` system user and needs specific file system permissions
...
<Note>
  The Unix permission sections and the `check-miru-access.sh` script apply to Linux only.
...
To upload data, the agent **reads** the files matched by a
[file rule's](/data-uploads/concepts/file-rules/overview) `source.glob` and streams them to
...
On Windows, the agent runs as the LocalSystem account and can read and write most paths
without additional configuration. To grant access explicitly, use `icacls`. For example,
to give the agent modify access to a config directory and everything inside it:

```powershell
icacls "C:\ProgramData\Robot\configs" /grant "SYSTEM:(OI)(CI)M"
```
...
- `--mode read-delete` — **[Data uploads](#deleting-local-files)** with a `retention`
  block. Requires `r` on the file and `r,w,x` on the parent
