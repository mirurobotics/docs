...
The examples below show the default `write` (Configs) mode. In `read-delete` and
`read-only` mode the write requirements drop per the mapping above — the file Write cell
becomes `-` in both, and the parent Write cell also becomes `-` in `read-only` — so a `-`
in those cells is expected and does not count against the result.
