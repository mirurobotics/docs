...
in the two cases that need it:

- **[Writing config instances](#writing-config-instances)** — the agent **writes** config
  instances to disk when it deploys a release.
- **[Reading files to upload](#reading-files-to-upload)** — the agent **reads** files on
  the device to upload them.
...
Unlike config writes, uploads are read-only — the `miru` user does **not** need write
access to the source files, unless you delete them after upload (below).
