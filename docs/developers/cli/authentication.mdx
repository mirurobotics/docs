...
Confirm the stored identity with [`miru whoami`](/references/cli/whoami).

## API key

The `login` command opens a browser so you can approve access, which makes it unsuitable for anything non-interactive. To authenticate without a prompt, export the `MIRU_API_KEY` environment variable instead.
