...
If a supported Miru Agent uses an API version, that API version is supported. You can find the supported Miru Agent versions on the [Agent Release Lifecycle](/docs/developers/agent/lifecycle) page.
...
For a history of changes to the Agent API, visit the Developer Changelog below and filter by the `Agent API` tag.
...
<Card
  ...
  arrow
>
  Release notes for each Agent API release.
</Card>
