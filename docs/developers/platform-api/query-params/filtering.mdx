...
Filtering by ID will return at most one resource, since IDs are unique. Other fields, like [config instance activity status](/docs/references/platform-api/latest/endpoints/config-instances/list#parameter-activity-status), can return multiple results.
