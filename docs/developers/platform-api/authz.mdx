...
| `devices:move` | move between groups |
| `devices:delete` | delete |

`devices:move` is checked against the group the devices are moving into, so the API key must be granted it on that group. Moving devices out of every group requires a workspace-level grant.
