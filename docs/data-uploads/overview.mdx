...
You define a small set of [**file rules**](/data-uploads/concepts/file-rules/overview) — what to
...
<img
  src="https://assets.mirurobotics.com/docs/v04/images/data-uploads/data-upload-flow.svg"
...
2. The Agent registers a new [upload](/data-uploads/concepts/uploads) with the Miru control plane.
3. Using credentials setup in the [bucket connection](/data-uploads/concepts/buckets#connect-a-bucket), the control plane issues a short-lived token valid only for the exact storage object key the file will be uploaded to.
...
held steady for the rule's [`stability_window_secs`](/data-uploads/concepts/file-rules/rule-definition#sources)
...
[deployment](/data-uploads/concepts/file-rules/overview#deployment) for details.


## Device data isolation

Devices' uploads are strictly isolated from each other. Isolation rests on a single requirement on the [upload
path](/data-uploads/concepts/file-rules/rule-definition#uploads): it must include the `{upload_id}` variable somewhere in the template.
...
<ParamField path="release_version" type="string">
  ...
  The [version](/concepts/releases#param-version) field of the release that was running when the file was detected.
