...
Once a [file rule](/data-uploads/concepts/file-rules/overview) with an `upload` block is
deployed to a device, the Miru Agent handles the rest: detecting when a matching file is
finished, uploading it to your bucket, and recording it in the ledger with the
properties below.
...
<ParamField path="device" type="Device">
  ...
  The [device](/concepts/devices/overview) that produced the upload.
</ParamField>

<ParamField path="file rule" type="File Rule">
  <ImmutableBadge />

  The [file rule](/data-uploads/concepts/file-rules/overview) that produced the upload.
</ParamField>

<ParamField path="deployment" type="Deployment">
  <ImmutableBadge />

  The [deployment](/concepts/deployments/overview) that was active on the device when the
  upload was produced.
</ParamField>
...
<ParamField path="file_path" type="string">
  ...
  variable in the [upload path template](/data-uploads/concepts/file-rules/rule-definition#uploads).
...
<ParamField path="bucket" type="Bucket">
  ...
  The registered [bucket](/data-uploads/concepts/buckets) the file was uploaded to.
</ParamField>
