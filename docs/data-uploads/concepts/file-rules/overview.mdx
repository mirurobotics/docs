...
[deployment](/concepts/deployments/overview) containing the rule reaches the `deployed`
[activity status](/concepts/deployments/status#activity-status) on *that device*.
