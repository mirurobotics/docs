...
The primary method of creating releases is via the command line. The dashboard does not support creating new releases (although you can [duplicate existing releases](/learn/releases/manage#duplicate-a-release)).
...
1. Config types annotations - each schema must be annotated with its [config type](/learn/config-types)
...
Schemas must be defined in a supported schema [language](/learn/schemas/languages/overview). You can find example schemas in our [getting-started repository](https://github.com/mirurobotics/getting-started).
