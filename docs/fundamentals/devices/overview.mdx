...
<ParamField path="current release" type="Release">
  ...
  The [release](/fundamentals/releases/overview#properties) to which the device's current deployment belongs.
...
<ParamField path="group" type="Group">
  ...
  The [group](/fundamentals/groups/overview#properties) to which the device belongs. If the device's group is not set, the group id is `null`.
...
The only exception to this is when a device is [reprovisioned](/fundamentals/devices/provision/reprovision). During reprovisioning, devices briefly enter the `activating` state before transitioning back to `online`.
