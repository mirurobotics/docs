...
The provisioning script is a legacy method for [provisioning](/learn/devices/provision/overview) devices. The provisioning script only supports installing `miru-agent` versions that *precede* [v0.9.0](/changelog/agent).
...
<Warning>
    The provisioning script only supports installing `miru-agent` versions that precede [v0.9.0](/changelog/agent). If installing Miru Agent `v0.9.0` or later, you must use [provisioning tokens](/learn/devices/provision/provisioning-tokens).
</Warning>
...
1. **Provisions the device** - registers the agent with Miru servers using the provisioning token
