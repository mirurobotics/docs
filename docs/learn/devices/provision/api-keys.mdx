...
Next, install the `miru-agent` package on the device you want to provision. If the `miru-agent` package is already installed, you can skip this step. For more information on installing the `miru-agent` package, visit the [installation](/docs/developers/agent/install) documentation.
...
Then execute the `provision` command bundled with the Miru Agent to create and activate a device using your API key.
