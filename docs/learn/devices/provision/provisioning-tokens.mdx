...
If you don't have an API key, follow the instructions for [creating an API key](/admin/apikeys#create-an-api-key) and select the `devices:provision` and `provisioning-tokens:write` scopes.
...
After retrieving the provisioning token, run the following command with the token.
