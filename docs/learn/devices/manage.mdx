...
4. Recreates the device's current deployment
...
A dialog will appear with a pre-authenticated command to reactivate the device using the `miru-agent` package's `activate` command.

Copy this command and paste it into a terminal of the device you want to reactivate.
