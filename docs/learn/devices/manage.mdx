...
Outside the reactivation capability, this installation command is identical to the activation command used to provision a device. For more information on the installation command, visit the [dashboard device provisioning](/learn/devices/provision#install-the-agent) documentation.

**Platform API**

To reactivate a device programmatically, use the [API key provisioning](/learn/devices/provision#api-keys) workflow with the [`--allow-reactivation`](/learn/devices/provision#param-allow-reactivation) flag enabled.
