...
Note that file paths which do not live in `/srv/miru/config_instances/` require Miru Agent [v0.8.0](/changelogs/agent#v0-8-0).
...
`v0.7.0` is the first CLI release with [CUE](/learn/schemas/languages/overview) support. You can now push CUE-based schemas directly and use CUE alongside existing JSON Schema workflows.
