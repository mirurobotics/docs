...
CLI `v0.10.0` specifies config instance file paths as an absolute path, allowing you to place config instances anywhere on the filesystem. File paths which do not live in `/srv/miru/config_instances/` require Miru Agent [v0.8.0](/docs/changelogs/agent#v0-8-0).
...
The default instance file path has been updated from `{config-type-slug}.json` (which previously pointed to `/srv/miru/config_instances/{config-type-slug}.json`) to `/srv/miru/configs/{config-type-slug}.json`.
