...
- The agent installer is now invoked as `provision` for first-time enrollment and `reprovision` for re-enrollment; device name is now a required field on enrollment
- Backend URL and MQTT host are now restricted to `mirurobotics.com` and its subdomains (loopback is also permitted); MQTT requires TLS except on loopback. If on-device settings are tampered with, the agent falls back to production defaults instead of going dark
- Pre-`v0.9.0` agent install scripts will refuse to install `v0.9.0` or later; follow the updated [install instructions](/docs/developers/agent/install)

## Features

- Added `provision` and `reprovision` subcommands for enrolling and re-enrolling devices using provisioning tokens
- On agent version change, per-version on-device state is reset and rebootstrapped from the backend so the device stays consistent with the backend across upgrades; the device's auth keypair is preserved so the device retains its identity
...
- `settings.log_level` now takes effect after the agent reloads its configuration, without requiring a restart
