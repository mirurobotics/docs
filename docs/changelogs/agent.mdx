...
- API key based provisioning is not supported for provisioning `miru-agent` versions `v0.9.0` or later; use [provisioning tokens](/docs/learn/devices/provision/provisioning-tokens) instead
<Warning>
    Legacy API key-based [install scripts](https://github.com/mirurobotics/agent/blob/main/scripts/install/provision.sh) will refuse to install `v0.9.0` or later; follow the updated [install instructions](/docs/developers/agent/install)
</Warning>
...
- Resources fetched via the [Device API](/docs/developers/device-api) are fetched from Miru servers if not found locally
