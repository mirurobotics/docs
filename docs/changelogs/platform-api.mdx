...
<Dropdown title="Activation tokens replaced by provisioning tokens">
  ...
  The `POST /devices/{device_id}/activation_token` endpoint has been removed. Instead, callers should migrate to using `POST /provisioning_tokens` instead.
  ...
  Provisioning tokens are not scoped to a pre-existing device — create a token first, then use it to provision a new device or activate an existing device. Visit the [provisioning tokens](/docs/learn/devices/provision/provisioning-tokens) documentation for more details.
...
<Steps>
  <Step title="Update the Miru-Version header">
    Set `Miru-Version: 2026-05-06.rainier` on all API requests. [Python SDK](/docs/developers/platform-api/sdks#python) `v0.10.0` marks the major version rollout of the `2026-05-06.rainier` API. For the latest SDK version compatibility information, visit the [Platform SDKs](/docs/developers/platform-api/sdks) page.
  </Step>

  <Step title="Migrate from activation tokens to provisioning tokens">
    Migrate install workflows to use [provisioning tokens](/docs/learn/devices/provision/provisioning-tokens) instead of activation tokens.
  </Step>
