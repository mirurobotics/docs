...
Provisioning tokens are only supported when installing `miru-agent` version `v0.9.0` or later. If installing a version that precedes `v0.9.0`, continue to use the [provisioning script](/essentials/devices/provision/provisioning-script).
...
To reassociate the machine with an existing Miru device, follow the [reprovisioning](/essentials/devices/provision/reprovision) documentation.
