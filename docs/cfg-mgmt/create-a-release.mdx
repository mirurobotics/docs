...
For [instance slots](/cfg-mgmt/primitives/schemas/overview#param-instance-slots), the hash covers each slot's key, file path, and required flag — but not its name or description. A schema whose only change is a slot's name or description is therefore identical to the schema you already have, and the change is not applied.

No two slots anywhere in a release may share a file path, since the config instances of a single deployment would otherwise compete for the same file on the device.
