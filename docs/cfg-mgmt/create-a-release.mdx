...
import { OptionalBadge, RequiredBadge } from '/snippets/components/requirement-badges.jsx';
...
The only way to create releases is via the command line. The dashboard does not support creating new releases (although you can [duplicate existing releases](/essentials/releases/manage#duplicate-a-release)).
...
The CLI is used to programmatically create releases in a [CI pipeline](/developers/ci/overview), ensuring releases are automatically created when code is pushed to a Git repository.

If you just want a concise overview of the CLI command and its flags, visit the [CLI Reference](/references/cli/release-create). For a comprehensive guide, continue reading below.
...
## Config schemas  <RequiredBadge />

[Config schemas](/cfg-mgmt/schemas/overview) aren't created separately from releases. Instead, the CLI creates the schemas in the same command as the release itself. Releases may contain any number of schemas, but they must contain at least one.
...
Schemas must be defined in a supported schema [language](/cfg-mgmt/schemas/languages/overview). You can find example schemas in our [getting-started repository](https://github.com/mirurobotics/getting-started).
...
<Tip>
  Miru supports CUE packages as a way to split a schema's definition into multiple files. Read more in the [CUE documentation](/cfg-mgmt/schemas/languages/cue#cue-packages).
</Tip>

**Schema Annotations**

Schemas support [annotations](/cfg-mgmt/schemas/overview#schema-annotations), which are Miru-specific extensions that define metadata about a schema.

The [`config type`](/cfg-mgmt/schemas/overview#schema-annotations) annotation is the only required annotation--it's how Miru knows which config type the schema belongs to. 

However, there are other useful annotations, such as the [`instance file path`](/cfg-mgmt/schemas/overview#schema-annotations) annotation, which defines where config instances for this schema are written to disk on your devices.

<Annotations />

## Upload rules  <OptionalBadge />

Similar to schemas, [upload rules](/data-uploads/upload-rules) are created as part of a release. Unlike schemas, upload rules are optional — a release may contain any number of them, including none.

Upload rules may belong to multiple releases and are identified by hashing their content. Upload rules with equivalent content are considered identical (even if comments, spacing, or other formatting is different).
...
Visit the [upload rules](/data-uploads/upload-rules) page for a complete reference of the upload rule fields and their usage.
