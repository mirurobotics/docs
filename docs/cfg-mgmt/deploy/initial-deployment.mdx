...
config schemas — one file for each [instance slot](/cfg-mgmt/primitives/schemas/overview#param-instance-slots) of each schema, so a schema with several
slots contributes several files.
