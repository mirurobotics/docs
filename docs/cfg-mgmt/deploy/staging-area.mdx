...
instance in the **Configurations** section of the panel. The deployment holds at most one
config instance per [instance slot](/cfg-mgmt/primitives/schemas/overview#param-instance-slots) of each of its schemas — and one for every slot marked
required — so a schema with several slots can appear here as several config instances.
