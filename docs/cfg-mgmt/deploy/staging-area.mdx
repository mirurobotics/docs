...
instance in the **Configurations** section of the panel. The deployment holds one config
instance per [instance slot](/cfg-mgmt/primitives/schemas/overview#param-instance-slots) of each of its schemas, so a schema with several slots
appears here as several config instances.
