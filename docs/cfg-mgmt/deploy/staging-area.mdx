...
To stage a deployment from the dashboard, click the **Stage** button in the top right. If there are no
...
<Frame>
  ![](https://assets.mirurobotics.com/docs/v04/images/releases/stage/dialog-stage.png)
</Frame>
...
<CliBadge command="deployment-clone" />
...
You can patch staged and [drifted](#deployment-drift) deployments. To apply a drift's
patches, patch the drifted deployment and make the same changes in the release editor.
...
[restaging](#review-a-deployment) the deployment, or apply it yourself by
[patching](#patch-a-deployment) the staged deployment and making the same changes.
...
The second option, in the dropdown next to **Restage**, is to
[archive](#archive-a-deployment) the deployment. This removes
