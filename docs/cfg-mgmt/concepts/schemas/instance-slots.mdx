...
An instance slot defines one or more valid config instances that are valid for a given schema. Instance slots are defined as an [annotation](/cfg-mgmt/concepts/schemas/overview#schema-annotations) within the schema itself.
