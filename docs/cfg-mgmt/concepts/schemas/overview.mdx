...
<ParamField path="instance slots" type="InstanceSlot[]">
  ...
  See [Instance slots](/cfg-mgmt/concepts/schemas/annotations/instance-slots) for fields, constraints, and default behavior.
</ParamField>
...
Annotations are a convenient way to store metadata with your schemas, such as the schema's config type and where its config instances are written on the device. Most annotations are optional, but some are required for Miru to process the schemas correctly.

For the full list of annotations and their syntax in each schema language, visit the [annotations](/cfg-mgmt/concepts/schemas/annotations/overview) section of this guide.
