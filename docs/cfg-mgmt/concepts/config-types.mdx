...
- New [releases](/concepts/releases) cannot be created with it
- New [schemas](/cfg-mgmt/concepts/schemas/manage) cannot be added to it

Existing schemas, releases, and deployments that use the config type are not
affected, and devices keep their current config instances. An archived config type can be
[unarchived](#unarchive-a-config-type) at any time.

To archive a config type, click the ellipses (...) on the config type and select
**Archive**.

<Frame>
  ![Archive Config Type Dialog](/images/config-types/ellipses-dropdown.png)
</Frame>

A dialog will appear. Hit **Archive** to confirm the archive.

<Frame>
  ![Archive Config Type Dialog](/images/config-types/archive-dialog.png)
</Frame>

## Unarchive a config type

Unarchiving removes the the archive restrictions placed on it, allowing releases and schemas to be created with it again. 

To unarchive a config type, click the ellipses (...) on the archived config type and select **Unarchive**.
...
Deleting a config type is only possible if there are no config instances for that config type. If a config type cannot be deleted, try [archiving](#archive-a-config-type) it instead.
