...
<img src="/snippets/devices/provision/ansible-control-flow.light.svg" alt="Provision role control flow: install or upgrade, stop if already provisioned, otherwise create a token and provision" className="block dark:hidden" />
{/* lint-ignore image-domain */}
<img src="/snippets/devices/provision/ansible-control-flow.dark.svg" alt="Provision role control flow: install or upgrade, stop if already provisioned, otherwise create a token and provision" className="hidden dark:block" />
...
Write a playbook to apply the role to the hosts in your inventory. 
...
<ParamField path="miru_agent_version" type="string" required>
  Miru Agent version to install. The version must be `0.10.2` or later. Use `latest` to install the newest available package on every run.
