...
To programmatically provision devices, Miru offers provisioning tokens. If you already manage hosts with Ansible, use the official [Ansible collection](/cfg-mgmt/provision-devices/ansible) instead of minting tokens yourself.
...
1. **Creates or activates the device** - creates a new device if the provided name does not exist, or activates an existing `inactive` device with that name. If the named device has already been activated (`online`, `offline`, and so on), the command fails. To attach a machine to an already-activated device, use the [reprovision](/cfg-mgmt/provision-devices/reprovision) flow.
