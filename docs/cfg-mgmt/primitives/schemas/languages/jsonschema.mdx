...
Every slot on the schema is validated against that same schema. Declare `x-miru-instance-slots` or `x-miru-instance-filepath`, but never both — declaring both is an error. Omitting both gives the schema a single default slot.
