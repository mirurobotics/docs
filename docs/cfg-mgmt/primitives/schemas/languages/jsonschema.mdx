...
<Framed 
    image="https://assets.mirurobotics.com/docs/v04/images/schema-langs/jsonschema/website.png" 
...
A JSON Schema declares [instance slots](/cfg-mgmt/primitives/schemas/overview#param-instance-slots) with the top-level `x-miru-instance-slots` keyword, an array of slot objects. Each slot is a separate file the schema writes on the device, so a single `motion-control` schema can configure several identical controllers:
...
Every slot on the schema is validated against that same schema. Declare `x-miru-instance-slots` or `x-miru-instance-filepath`, but never both — declaring both is an error. Omitting both gives the schema a single default slot. If `x-miru-instance-slots` is present it must list at least one slot, so omit the keyword entirely rather than writing an empty array.
