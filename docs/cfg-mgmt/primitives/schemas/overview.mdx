...
<ParamField path="instance slots" type="InstanceSlot[]">
  ...
  - `name` contains between 1 and 48 characters; any markup is stripped
  - `filepath` contains between 1 and 128 characters and no `..` segments
  - `description` contains at most 512 characters; any markup is stripped, and a description consisting only of markup is discarded
...
<ParamField path="instance format" type="string">
  ...
  When a schema does not declare an instance format, the format is inferred from the extension of its first slot's file path, and every one of its slot file paths must imply that same format. It therefore defaults to `json` under the default instance file path. A schema that declares an instance format explicitly is not subject to that matching requirement, and its slot file paths' extensions are never compared.
