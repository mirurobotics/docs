...
<ParamField path="instance file path" type="string">
  ...
  Instance file paths control the type of file that config instances are deployed as. Currently, JSON (`.json`) and YAML (`.yaml`, `.yml`) are supported. [Opaque](/cfg-mgmt/primitives/schemas/languages/opaque) schemas may use any file extension, since their instance content is not validated.
...
<ParamField path="instance format" type="string">
  ...
  The instance format is inferred from the instance file path's extension, so it defaults to `json` under the default instance file path.
...
- [Opaque](/cfg-mgmt/primitives/schemas/languages/opaque) (metadata-only schemas for configs Miru does not validate)
