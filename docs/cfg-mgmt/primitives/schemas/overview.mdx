...
<ParamField path="instance slots" type="InstanceSlot[]">
  ...
  - `filepath` contains between 1 and 128 bytes and no `..` anywhere in the path, and on a JSON Schema or CUE schema ends in `.json`, `.yaml`, or `.yml` — [opaque](/cfg-mgmt/primitives/schemas/languages/opaque) schemas accept any extension
...
<ParamField path="instance format" type="string">
  ...
  When a schema does not declare an instance format, the format is inferred from the extension of its first slot's file path, and every one of its slot file paths must imply that same format. It therefore defaults to `json` under the default instance file path, or to `yaml` when the schema is written in YAML and takes the YAML default path. A schema that declares an instance format explicitly is not subject to that matching requirement, though on a JSON Schema or CUE schema every slot file path must still end in `.json`, `.yaml`, or `.yml`. Opaque schemas are exempt, so their slot file paths may carry any extension.
