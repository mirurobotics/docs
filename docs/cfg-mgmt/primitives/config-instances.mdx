...
<ParamField path="file path" type="string">
  ...
  The file path comes from the [instance slot](/cfg-mgmt/primitives/schemas/overview#param-instance-slots) the instance is bound to; it is not set on the instance itself.
...
<ParamField path="slot key" type="string">
  ...
  The key of the [instance slot](/cfg-mgmt/primitives/schemas/overview#param-instance-slots) on the config schema that this instance fills.

  Every config instance is bound to exactly one slot, and takes that slot's file path as its own. A schema that declares no slots has a single slot with the key `default`.
