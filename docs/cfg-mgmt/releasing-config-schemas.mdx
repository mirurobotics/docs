...
The primary method of creating releases is via the command line. The dashboard does not support creating new releases (although you can [duplicate existing releases](/fundamentals/releases/manage#duplicate-a-release)).
...
Furthermore, the CLI can be used to programmatically create releases in a [CI pipeline](/developers/ci/overview), ensuring releases contain the correct schemas and are always associated with the correct Git commit.

If you've created a release via the CLI before and want a concise overview of the command and its flags, visit the [CLI Reference](/references/cli/release-create). For a comprehensive guide, continue reading below.
...
1. Config types annotations - each schema must be annotated with its [config type](/cfg-mgmt/config-types)
